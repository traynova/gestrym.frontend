...
import { Mail, Loader2, AlertCircle, ArrowLeft, ChevronRight, CheckCircle2 } from 'lucide-react';
...
export function ForgotPasswordPage() {
  ...
  const onSubmit = async (data: RecoveryForm) => {
    ...
    setErrorDesc(null);
    try {
      await authApi.requestPasswordRecovery(data);
      setIsSuccess(true);
    } catch (err: any) {
      setErrorDesc(err.response?.data?.error || 'No se pudo procesar la solicitud. Verifica tu correo.');
    } finally {
      setIsLoading(false);
    }
  };
            ...
            {!isSuccess ? (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex flex-col">
                       ...
                       <h2 className="text-3xl font-extrabold text-white tracking-tight">Recuperar</h2>
                    </div>
                  </div>
                  <p className="text-slate-400">Ingresa tu correo para recibir las instrucciones de recuperación.</p>
                </div>

                <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
                  {errorDesc && (
                    <div className="p-4 text-sm text-red-400 bg-red-900/10 border border-red-500/20 rounded-2xl flex items-center gap-3">
                  ...
                  <button type="submit" disabled={isLoading} className="w-full flex items-center justify-center py-4 px-4 rounded-2xl text-base font-bold text-white bg-red-600 hover:bg-red-500 shadow-lg shadow-red-600/20 active:scale-[0.98] transition-all disabled:opacity-50">
                    {isLoading ? (
                      ...
                      <>Enviar instrucciones <ChevronRight className="w-5 h-5 ml-2" /></>
                    )}
                  </button>
                </form>
              </motion.div>
              ...
              >
                ...
                <div className="space-y-2">
                  <h2 className="text-3xl font-extrabold text-white">¡Correo enviado!</h2>
                  <p className="text-slate-400">Revisa tu bandeja de entrada. Te hemos enviado los pasos para restablecer tu contraseña.</p>
                </div>
                <Link to="/login" className="w-full flex items-center justify-center py-4 px-4 rounded-2xl text-base font-bold text-white bg-slate-800 hover:bg-slate-700 transition-all">
                  Volver al login
                </Link>
              </motion.div>
            )}
      ...
      <div className="hidden lg:block lg:flex-1 relative overflow-hidden bg-slate-900 border-l border-slate-800/50">
        ...
        <div className="absolute bottom-12 left-12 right-12 z-20">
          <div className="bg-slate-950/60 backdrop-blur-xl border border-white/10 p-8 rounded-[2.5rem] shadow-2xl">
            <h3 className="text-3xl font-extrabold text-white mb-4">No te detengas, <br/><span className="text-red-500">recupera tu acceso</span></h3>
            <p className="text-slate-400 text-lg font-light leading-relaxed">Tu proceso no tiene por qué pausarse. Estamos aquí para ayudarte a volver al ruedo.</p>
          </div>
        </div>
      </div>
    </div>
